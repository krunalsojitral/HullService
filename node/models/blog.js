...
function User() {
    ...
    this.getUnpaidBlogList = function (callback) {
        connection.acquire(function (err, con) {
            con.query('SELECT *,blog.created_at as blog_date FROM blog where blog.draft_status = $1 and blog.status = $2 and (role ILIKE $3) order by blog.blog_id desc', [0, 1, '%4%'], function (err, result) {
                con.release()
                if (err) {
                    if (env.DEBUG) { console.log(err); }
                    callback(err, null);
