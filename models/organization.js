...
function Organization() {
  ...
  this.getAllAdminorganization = function (status, callback) {
    connection.acquire(function (err, con) {
      ...
      if (status) {
        sql =
          "SELECT * FROM organization where status = $1 order by UPPER(organization_name) ASC";
        array = [status];
      } else {
        sql =
          "SELECT * FROM organization order by UPPER(organization_name) ASC";
      }
      ...
      con.query(
        ...
        function (err, result) {
          if (result.rows.length === 0) {
            const sql =
              "INSERT INTO organization(organization_name,status) VALUES($1,$2) RETURNING *";
  ...
  this.updateorganizationByadmin = function (
    record,
    organization_id,
    update_value,
    callback
      ...
      organization.map((data) => {
        con.query(
          ...
          function (err, results) {
            con.query(
              "UPDATE users SET organization = NULL WHERE organization = $1",
              [data.organization_id],
              function (err, result) {}
            );
          }
