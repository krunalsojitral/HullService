...
function User() {
    ...
    this.getPaidBlogList = function (role, user_id, callback) {
        connection.acquire(function (err, con) {            
            ...
            con.query('SELECT *,blog.blog_id as b_id, blog.created_at as blog_date FROM blog left join bookmark_blog on blog.blog_id = bookmark_blog.blog_id and bookmark_blog.user_id = $1 left join blog_order on blog.blog_id = blog_order.blog_id and blog_order.user_id = $2 where blog.draft_status IS NULL and blog.status = $3 and (blog.role ILIKE $4 or blog.role ILIKE $5) order by blog.blog_id desc', [user_id, user_id, 1, '%' + role + '%', '%4%'], function (err, result) {
                con.release()
                if (err) {
                    if (env.DEBUG) { console.log(err); }
                    callback(err, null);
