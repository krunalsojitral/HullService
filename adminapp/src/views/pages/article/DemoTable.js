...
const DemoTable = () => {
  ...
  const updateItemStatus = (item, status) => {
    if (status == 1) {
      var message = 'Are you sure you want to activate the article ?'
    } else {
      var message = 'Are you sure you want to deactivate the article ?'
    }
    Swal.fire({
      //title: 'warning!',
      icon: 'warning',
  ...
  const getNewList = (status) => {
    axios.get(api_url + '/article/articleList?status=' + status, {}).then((result) => {
      if (result.data.status) {
            ...
            status: (item) => (
              <td className="tooltip-box">
                {item.status === 1 ? (
                  ...
                  >
                    Active
                    <span className="tooltip-title">De-activating the article will remove the article from the front end.</span>
                  </p>
                ) : (
                  <p
                    onClick={() => {
                  ...
                  >
                      Inactive
                      <span className="tooltip-title">Activating the article will add the article back on the front end.</span>
                  </p>
                )}
