...
const DemoTable = () => {
  ...
  const getNewList = (status) => {
    axios.get(api_url + '/academicdiscipline/academicdisciplineList?status=' + status, {}).then((result) => {
      if (result.data.status) {
  ...
  const deleteItem = (e) => {
    ...
    if (filteredThatArray.length > 0) {
      ...
      Swal.fire('Oops...', 'Please select academic discipline', 'error')
    }
  }
        ...
        <CDataTable
          ...
          scopedSlots={{
            checkbox: (item, index) => (
              <td>
                <input
                  key={item.academicdiscipline_id}
                  name={index}
                  type="checkbox"
                ...
                return (
                  <td className="py-2">
                    <CButton
                      ...
                      shape="square"
                      size="sm"
                      onClick={() => history.push(`/academicdisciplineedit/${item.academicdiscipline_id}`)}
                      className="mr-1"
                    > Edit
                  </CButton>
                  </td>
