...
const DemoTable = () => {
  ...
  const fields = [
    { key: 'partner', _style: { width: '20%'} },
    // { key: 'partner_url', _style: { width: '20%' } },
    {
      key: 'show_details',
      label: '',
      _style: { width: '1%' },
  ...
  const deletepartner = (partner_id) => {
    ...
    Swal.fire({
      ...
      if (results.isConfirmed) {
        var obj = { partner_id: partner_id }
        axios.post(api_url + '/cms/deletepartner', obj).then((result) => {
          if (result.data.status) {
            getNewListWrap();
  ...
  const addInformationAct = (data) => {
    ...
    axios.post(api_url + "/cms/addpartnerByadmin", formData, {})
      .then((result) => {
        if (result.data.status) {
        ...
        scopedSlots = {{  
          'partner':
            item => {
              return (
                <td className="py-2">
                  <img width="50px" height="50px" src={item.partner_url}/>
                </td>
              )},
          'show_details':
            item => {
              return (
                <td className="py-2">
                  ...
                  <CButton
                    ...
                    shape="square"
                    size="sm"
                    onClick={() => deletepartner(item.partner_id)}
                    className="mr-1"
                  > Delete
                  </CButton>
