        ...
        axios.post(api_url + "/tag/changetagStatus", obj)
          .then((result) => {
            if (result.data.status) {
              ...
              var successmessage = '';
              if (status == 1) {
                successmessage = 'You have successfully activated tag.'
              } else {
                successmessage = 'You have successfully deactivated tag.'
              }
  ...
  const deleteItem = (e) => {
    ...
    if (filteredThatArray.length > 0) {
      ...
      Swal.fire({
        //title: 'warning!',
        icon: 'warning',
        text: 'Are you sure you want to delete the selected tag(s) ?',
        confirmButtonText: `Yes`,
        showCancelButton: true,
        cancelButtonText: 'No',
        cancelButtonColor: '#e57979',
      ...
      Swal.fire('Oops...', 'Please select tag', 'error')
    }
  }
