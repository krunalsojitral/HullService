        ...
        axios.post(api_url + "/occupation/changeOccupationStatus", obj)
          .then((result) => {
            if (result.data.status) {
              ...
              var successmessage = '';
              if (status == 1) {
                successmessage = 'You have successfully activated occupation.'
              } else {
                successmessage = 'You have successfully deactivated occupation.'
              }
  ...
  const deleteItem = (e) => {
    ...
    if (filteredThatArray.length > 0) {
      ...
      Swal.fire({
        //title: 'warning!',
        icon: 'warning',
        text: 'Are you sure you want to delete the selected occupation(s) ?',
        confirmButtonText: `Yes`,
        showCancelButton: true,
        cancelButtonText: 'No',
        cancelButtonColor: '#e57979',
