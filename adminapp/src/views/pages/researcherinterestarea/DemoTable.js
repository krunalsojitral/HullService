...
const DemoTable = () => {
  ...
  const updateItemStatus = (indexs, item, status) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        var obj = {
          researcherinterestarea_id: item.researcherinterestarea_id,
          status: status,
        };
  ...
  const getNewList = (status) => {
    axios.get(api_url + '/researcherinterestarea/researcherinterestareaList?status=' + status, {}).then((result) => {
      if (result.data.status) {
  ...
  const deleteItem = (e) => {
    const filteredThatArray = items.filter((item) => item.isChecked == true).map(item => {
      const container = {};
      container['researcherinterestarea_id'] = item.researcherinterestarea_id;
      return container;
    });
    ...
    if (filteredThatArray.length > 0) {
      ...
      Swal.fire({
        //title: 'warning!',
        icon: 'warning',
        text: 'Are you sure you want to delete the selected interest area(s) ?',
        confirmButtonText: `Yes`,
        showCancelButton: true,
        cancelButtonText: 'No',
        cancelButtonColor: '#e57979',
        ...
        if (result.isConfirmed) {
          axios.post(api_url + '/researcherinterestarea/deleteResearcherinterestarea', { researcherinterestarea: filteredThatArray }).then((result) => {
            if (result.data.status) {
              getNewListWrap('');
  ...
  return (
    ...
    <div>
       <CCardHeader className="custom-table-header">
            <div>&nbsp;&nbsp; Researcher Interest Area</div>
        ...
        <CDataTable
          ...
          scopedSlots={{
            checkbox: (item, index) => (
              <td>
                <input
                  key={item.researcherinterestarea_id}
                  name={index}
                  type="checkbox"
                ...
                return (
                  <td className="py-2">
                    <CButton
                      ...
                      shape="square"
                      size="sm"
                      onClick={() => history.push(`/researcherinterestareaedit/${item.researcherinterestarea_id}`)}
                      className="mr-1"
                    > Edit
                  </CButton>
                  </td>
