...
const DemoTable = () => {
  ...
  const updateItemStatus = (indexs, item, status) => {
    var message = '';
    if (status == 1) {
      message = 'Are you sure you want to activate an organization ?'
    } else {
      message = 'Are you sure you want to deactivate an organization ?'
    }
    Swal.fire({
      //title: 'warning!',
      icon: 'warning',
        ...
        axios.post(api_url + "/organization/changeOrganizationStatus", obj)
          .then((result) => {
            if (result.data.status) {
              ...
              var successmessage = '';
              if (status == 1) {
                successmessage = 'You have successfully activated organization.'
              } else {
                successmessage = 'You have successfully deactivated organization.'
              }
  ...
  const deleteItem = (e) => {
    ...
    if (filteredThatArray.length > 0) {
      ...
      Swal.fire({
        //title: 'warning!',
        icon: 'warning',
        text: 'Are you sure you want to delete the selected organization(s) ?',
        confirmButtonText: `Yes`,
        showCancelButton: true,
        cancelButtonText: 'No',
        cancelButtonColor: '#e57979',
