  ...
  Forum.getAllAdminforum(status, function (err, result) {
    if (err) {
      ...
      var forumList = result.map((data) => {
        ...
        retObj["created_on"] = data.forum_date
          ? moment(data.forum_date).format("YYYY-MM-DD")
          : "";
        retObj["status"] = data.forum_status;
        retObj["user_status"] = data.user_status;
        retObj["retire"] = data.retire;
        retObj["created_by"] = data.first_name
  ...
  asyn.waterfall(
            ...
            if (data.length > 0) {
              let temparray = new Promise(async (resolve, reject) => {
                for (let datas of data) {
                  await Forum.getCommentCount(
                    datas.forum_comment_id,
                    function (err, cntdata) {
                      if (cntdata.length > 0) {
              ...
              temparray.then((finalresult) => {
                var forumCommentList = finalresult.map((data) => {
                  ...
                  retObj["user_name"] = data.first_name
                    ? data.first_name
                    : "" + " " + data.last_name
                    ? data.last_name
                    : "";
    ...
    function (error, finalData) {
      if (error) {
        ...
        return res.json({
          status: 1,
          response: { data: finalData, msg: "data found" },
        });
      }
    }
  );
      ...
      asyn.waterfall(
        ...
        function (error, video) {
          if (error) {
            ...
            return res.json({
              status: 1,
              response: { data: video, msg: "data found" },
            });
          }
        }
      );
    ...
    if (!errors.isEmpty()) {
      ...
      Forum.addforumByadmin(record, tag, function (err, data) {
        if (err) {
          ...
          return res.json({
            status: 1,
            response: { msg: "Forum added successfully.", data: data },
          });
        }
      });
    }
  ...
  Forum.updateforumByadmin(
    ...
    function (err, data) {
      if (err) {
        ...
        return res.json({
          status: 1,
          response: { msg: "Forum updated successfully.", data: data },
        });
      }
    }
  );
...
router.post(
  "/deleteForum",
  [check("comment_id", "comment id is required").notEmpty()],
  (req, res, next) => {
    ...
    Forum.deleteForum(comment_id, function (err, data) {
      if (err) {
        ...
        return res.json({
          status: 1,
          response: { msg: "Comment(s) deleted successfully.", data: data },
        });
      }
    });
  }
...
router.post("/getForumHeadingList", function (req, res) {
  ...
  Forum.getForumHeadingList(function (err, results) {
    if (err) {
      return res.json({
        status: 0,
        response: { data: [], msg: "data not found" },
      });
    } else {
            ...
            if (results.length > 0) {
              let temparray = new Promise(async (resolve, reject) => {
                for (let heading of results) {
                  await Forum.getForumListByForumHeading(
                    heading.forumheading_id,
                    search,
                    forum_id,
                    function (err, data) {
              ...
              let temparray = new Promise(async (resolve, reject) => {
                for (let datas of forum) {
                  for (let dataf of datas.forum) {
                    await Forum.getLastComment(
                      dataf.forum_id,
                      function (err, data) {
                        if (data.length > 0) {
                          var forumList = data.map((comment) => {
                            let retObj = {};
                            retObj["created_at"] = helper.timeSince(
                              comment.created_at
                            );
                            retObj["forum_comment_count"] =
                              comment.forum_comment_count;
                            return retObj;
                          });
    ...
    if (!errors.isEmpty()) {
      ...
      asyn.waterfall(
        [
          function (done) {
            if (req.body.search) {
              Forum.getSubForumTagSearchList(
                forum_heading_id,
                search,
                function (err, data) {
                  if (err) {
          ...
          function (forum_id, done1) {
            Forum.getSubForumListByForumHeading(
              forum_heading_id,
              search,
              forum_id,
...
router.get("/getForumTagList", function (req, res) {
  ...
  Forum.getForumTagList(function (err, results) {
    if (err) {
      ...
      return res.json({
        status: 1,
        response: { msg: "Tag list successfully.", data: newarray },
      });
    }
  });
});
    ...
    if (!errors.isEmpty()) {
      ...
      Forum.getSubForumTagList(forum_heading_id, function (err, results) {
        if (err) {
          ...
          var newarray = [
            ...new Map(results.map((v) => [v.tag_id, v])).values(),
          ];
          return res.json({
            status: 1,
            response: { msg: "Tag list successfully.", data: newarray },
          ...
          Forum.updateCommentForumRequest(obj, function (err, updateresult) {
            if (err) {
              ...
              var htmlUser = fs.readFileSync(
                __dirname + "/templates/forumRequest/approved_forum.html",
                "utf8"
              );
              var dynamicHtml = {
                logo: logo,
      ...
      Forum.getforumRequestDataById(forum_id, function (err, result) {
        ...
        Forum.updateComment(obj, function (err, results) {
          if (err) {
            ...
            var htmlUser = fs.readFileSync(
              __dirname + "/templates/forumRequest/reject_forum.html",
              "utf8"
            );
            var dynamicHtml = {
              logo: logo,
    ...
    asyn.waterfall(
          ...
          Forum.getforumRequestDataById(forum_id, function (err, result) {
            if (err) {
              ...
              if (result.length > 0) {
                ...
                // }
                comment.started = moment(result[0].forum_date).format(
                  "MMMM Do, YYYY"
                );
      ...
      function (error, comment) {
        if (error) {
          ...
          return res.json({
            status: 1,
            response: { data: comment, msg: "data found" },
          });
        }
      }
    );
      ...
      Forum.followUser(obj, function (err, result) {
        if (err) {
          ...
          if (result) {
            return res.json({
              status: 1,
              response: { data: result, msg: "Data found" },
            });
          } else {
            return res.json({
              status: 0,
        ...
        if (err) {
          ...
          if (result) {
            if (req.body.subcomment && req.body.subcomment == "subcomment") {
              Forum.getForumSubReplyComment(
                obj.parent_comment_id,
                imageLink,
                function (err, mainresult) {
                  return res.json({
                    status: 1,
                    response: { data: mainresult, msg: "Data found" },
                  });
                }
              );
              ...
              if (req.body.parent_comment_id) {
                Forum.getForumReplyComment(
                  ...
                  function (err, data) {
                    return res.json({
                      status: 1,
                      response: { data: data.reply_list, msg: "Data found" },
                    });
                  }
                );
              } else {
                User.getUserById(userDetail.id, function (err, userdata) {
                  var user_obj = {
                    ...
                    created_on: moment(result[0].created_at).format(
                      "MMMM Do, YYYY"
                    ),
                    first_name: userDetail.first_name,
                    last_name: userDetail.last_name,
                  ...
                  return res.json({
                    status: 1,
                    response: { data: user_obj, msg: "Data found" },
                  });
                });
              }
            }
    ...
    if (!errors.isEmpty()) {
      ...
      Forum.updateComment(obj, function (err, result) {
        if (err) {
          ...
          return res.json({
            status: 1,
            response: { data: result, msg: "Comment updated successfully." },
          });
        }
      });
    }
    ...
    if (!errors.isEmpty()) {
      ...
      Forum.deleteComment(req.body.forum_comment_id, function (err, result) {
        if (err) {
          ...
          return res.json({
            status: 1,
            response: { data: result, msg: "Comment(s) deleted successfully." },
          });
        }
      });
    }
      ...
      Forum.forumCommentLike(obj, function (err, result) {
        if (err) {
          ...
          if (result.length > 0) {
            return res.json({
              status: 1,
              response: { data: "like", msg: "Data found" },
            });
          } else {
            return res.json({
              status: 1,
              response: { data: "unlike", msg: "Data not found" },
            });
          }
        }
      });
...
router.post("/updateForumContent", function (req, res) {
  ...
  Forum.updateForumContent(obj, function (err, data) {
    if (err) {
      ...
      return res.json({
        status: 1,
        response: { msg: "Forum content updated successfully.", data: data },
      });
    }
  });
});
    ...
    if (!errors.isEmpty()) {
      ...
      Forum.deleteMultipleForum(forum, function (err, result) {
        if (err) {
          ...
          return res.json({
            status: 1,
            response: { msg: "Thread(s) deleted successfully", data: result },
          });
        }
      });
    }
...
router.get("/getForumNotificationCount", function (req, res) {
  ...
  Forum.getForumNotificationCount(function (err, Count) {
    if (err) {
      ...
      return res.json({
        status: 1,
        response: { data: Count, msg: "data found" },
      });
    }
  });
});
      ...
      Forum.forumReport(forum_comment_id, user_id, function (err, result) {
        if (err) {
          ...
          if (result.length > 0) {
            return res.json({
              status: 1,
              response: { msg: "Reported successfully", data: result },
            });
          } else {
            return res.json({
              status: 1,
              response: { msg: "Unreport successfully", data: result },
            });
          }
        }
      });
