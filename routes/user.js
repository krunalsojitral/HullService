    ...
    asyn.waterfall([
            ...
            if (data[0].role == 4) {
              let record = {
                ...
                academic_discipline: req.body.academic_discipline
                  ? req.body.academic_discipline
                  : "",
                research_description: req.body.research_description
                  ? req.body.research_description
                  : "",
                ...
                other_academic_discipline: req.body.other_academic_discipline
                  ? req.body.other_academic_discipline
                  : "",
                user_read_status: 0,
              };
      ...
      function (overview, done2) {
        if (overview.other_data.changeStatus == "yes") {
          ...
          let update_value = [
            overview.data.first_name,
            overview.data.last_name,
            overview.data.email,
            overview.data.organization,
            overview.data.academic_discipline,
            overview.data.research_description,
            overview.data.about_us,
            overview.data.phone,
            overview.data.status,
            overview.data.role,
            overview.data.email_verification_token,
            overview.data.created_at,
            overview.data.other_academic_discipline,
            overview.data.user_read_status,
          ];
            ...
            function (err, datas) {
              if (err) {
                ...
                return res.json({
                  status: 1,
                  response: {
                    msg: "Your request was successful. We have sent you a confirmation link to your email address. Please click on the link to verify/validate your account. Thank You!.",
                  },
                });
              }
            }
            ...
            bcrypt.genSalt(10, (err, salt) => {
              bcrypt.hash(req.body.password, salt, (err, hash) => {
                let record = {
                  ...
                  email_verification_token: shortid.generate() + Date.now(),
                  created_at: moment.utc().format("YYYY-MM-DD"),
                  professional_interest_of_area:
                    req.body.professional_interest_of_area,
                  researcher_interest_of_area:
                    req.body.researcher_interest_of_area,
                  other_sector: req.body.other_sector
                    ? req.body.other_sector
                    : "",
                  other_academic_discipline: req.body.other_academic_discipline
                    ? req.body.other_academic_discipline
                    : "",
                  other_occupation: req.body.other_occupation
                    ? req.body.other_occupation
                    : "",
                  other_professional_interest_area: req.body
                    .other_professional_interest_area
                    ? req.body.other_professional_interest_area
                    : "",
                  other_research_interest_area: req.body
                    .other_research_interest_area
                    ? req.body.other_research_interest_area
                    : "",
        ...
        User.addUser(overview.data, async function (err, data) {
          if (err) {
            ...
            return res.json({
              status: 1,
              response: {
                msg: "Your payment was successful. We have sent you a confirmation link to your email address. Please click on the link to verify/validate your account. Thank You!.",
              },
            });
          }
        });
        ...
        User.checkUserRegistration(email, function (err, data) {
          ...
          if (totalrecord) {
            if (data[0].role == 3) {
              return res.json({
                status: 0,
                response: { msg: "You are already register as researcher." },
              });
            } else {
              if (data[0].role == 4 && !data[0].password) {
                      ...
                      function (err, datas) {
                        if (err) {
                          ...
                          if (hostname == env.LOCAL_HOST_USER_APP) {
                            ...
                            resetLink =
                              env.APP_URL +
                              "activation-account?activationcode=" +
                              token +
                              "&email=" +
                              data[0].email;
                          ...
                          var htmlUser = fs.readFileSync(
                            __dirname +
                              "/templates/userRegistration/verifyEmail.html",
                            "utf8"
                          );
                          ...
                          return res.json({
                            status: 1,
                            response: { msg: "There was an email error" },
                          });
                        }
                      }
                    );
                ...
                return res.json({
                  status: 0,
                  response: { msg: "You are already register." },
                });
              }
            }
          } else {
  ...
  (req, res) => {
    ...
    User.getUserById(user_id, function (err, data) {
      ...
      if (hostname == env.LOCAL_HOST_USER_APP) {
        home_url = env.APP_URL;
        resetLink =
          env.APP_URL + "register?code=" + data[0].email_verification_token;
        admin_app_url = env.ADMIN_APP_URL;
      } else {
        home_url = env.APP_URL;
        resetLink =
      ...
      var htmlUser = fs.readFileSync(
        __dirname + "/templates/userRegistration/userRegistration.html",
        "utf8"
      );
      ...
      transporter.sendMail(mailOptions1, (error, info) => {
        if (error) {
          ...
          return res.json({
            status: 1,
            response: {
              msg: "We have sent confirmation link to user for email verification.",
            },
          });
        }
      });
...
router.post(
  ...
  (req, res, next) => {
    ...
    if (!errors.isEmpty()) {
      ...
      User.checkEmailVerifyUser(
        email_verify_token,
        email,
        function (req, result) {
          if (result.length) {
            let record1 = { status: 1 };
            User.emailTokenUpdate(
              record1,
              email_verify_token,
              function (err, data) {
                if (err) {
            ...
            return res.json({
              status: 0,
              response: { msg: "Reset token is invalid." },
            });
          }
        }
      );
        ...
        if (err) {
          ...
          if (totalrecord) {
            if (data[0].password) {
              ...
              return res.json({
                status: 0,
                response: { msg: "Please register first." },
              });
            }
          } else {
      ...
      User.checkUserRegistration(email, (err, data) => {
        ...
        if (data.length) {
          ...
          User.userProfileUpdate(record, user_id, function (req, result) {
            ...
            var htmlUser = fs.readFileSync(
              __dirname + "/templates/resetPassword/resetPassword.html",
              "utf8"
            );
            var dynamicHtml = {
              logo: logo,
            ...
            transporter.sendMail(mailOptions, (error, info) => {
              if (error) {
                ...
                return res.json({
                  status: 1,
                  response: {
                    msg: "We have emailed you a link to reset your password.",
                  },
                });
              }
            });
          ...
          return res.json({
            status: 0,
            response: { msg: "We can't find a user with this Email" },
          });
        }
      });
    }
              ...
              User.passwordUpdate(
                ...
                function (err, data) {
                  if (err) {
                    ...
                    return res.json({
                      status: 1,
                      response: { msg: "Password is updated successfully." },
                    });
                  }
                }
              );
      ...
      User.checkUserRegistration(email, function (err, data) {
        if (err) {
          ...
          if (totalrecord) {
            return res.json({
              status: 0,
              response: { msg: "Sorry, email already exists." },
            });
          } else {
            return res.json({
              status: 1,
              response: { msg: "Email is not exists." },
            });
          }
        }
      });
    ...
    if (err) {
      ...
      let temparray = new Promise(async (resolve, reject) => {
        for (let userdata of result) {
          await User.getUserInterestAreaById(
            userdata.id,
            userdata.role_id,
            function (err, interestresult) {
              if (interestresult && interestresult.length > 0) {
                userdata.csvString = Array.from(
                  interestresult.values(),
                  (v) => v.name
                ).join(", ");
              } else {
                userdata.csvString = "";
      ...
      User.getUserById(user_id, function (err, result) {
        if (err) {
          ...
          if (result != "") {
            ...
            userList["avatar"] = result[0].user_image
              ? imageLink + env.USER_VIEW_PATH_THUMB + result[0].user_image
              : "";
            return res.json({
              status: 1,
              response: { data: userList, msg: "data found" },
            });
          } else {
            return res.json({
              status: 1,
              response: { data: {}, msg: "data found" },
            });
          }
        }
      });
    ...
    if (!errors.isEmpty()) {
      ...
      User.getAdminUserById(user_id, function (err, result) {
        if (err) {
          ...
          if (result != "") {
            User.getUserInterestAreaById(
              user_id,
              result[0].role_id,
              function (err, interestresult) {
                var csvString = "";
                if (interestresult && interestresult.length > 0) {
                  csvString = Array.from(
                    interestresult.values(),
                    (v) => v.name
                  ).join(", ");
                } else {
                ...
                userList["avatar"] = result[0].user_image
                  ? imageLink + env.USER_VIEW_PATH_THUMB + result[0].user_image
                  : "";
                userList["academicdisciplinename"] =
                  result[0].academicdisciplinename;
                userList["level_of_education"] = result[0].level_of_education;
                userList["other_sector"] = result[0].other_sector;
                userList["other_academic_discipline"] =
                  result[0].other_academic_discipline;
                userList["other_occupation"] = result[0].other_occupation;
                userList["other_professional_interest_area"] =
                  result[0].other_professional_interest_area;
                userList["other_research_interest_area"] =
                  result[0].other_research_interest_area;
                userList["pinterestarea"] = csvString;
                userList["about_us"] = result[0].about_us;
                userList["phone"] = result[0].phone;
                userList["research_description"] =
                  result[0].research_description;
                userList["email_verification_token"] =
                  result[0].email_verification_token;
                userList["first_time_login"] = result[0].first_time_login;
                userList["joined_date"] = result[0].joined_date
                  ? moment(result[0].joined_date).format("MM-DD-YYYY")
                  : "";
                userList["renewal_date"] = result[0].renewal_date
                  ? moment(result[0].renewal_date).format("MM-DD-YYYY")
                  : "";
                return res.json({
                  status: 1,
                  response: { data: userList, msg: "data found" },
        ...
        function (overview, done1) {
          User.adduserByadmin(overview.data, function (err, data) {
            if (err) {
              ...
              return res.json({
                status: 1,
                response: { msg: "User added successfully.", data: data },
              });
            }
          });
        },
  ...
  function (req, res) {
    ...
    User.updateFirstView(user_id, function (err, data) {
      if (err) {
        ...
        return res.json({
          status: 1,
          response: { msg: "User updated successfully.", data: data },
        });
      }
    });
  }
                ...
                if (error) {
                  ...
                  if (hostname == env.LOCAL_HOST_USER_APP) {
                    ...
                    resetLink =
                      env.APP_URL +
                      "activation-account?activationcode=" +
                      finalData[0].email_verification_token +
                      "&email=" +
                      finalData[0].email;
                    admin_app_url = env.ADMIN_APP_URL;
                  } else {
                  ...
                  var htmlUser = fs.readFileSync(
                    __dirname + "/templates/userRegistration/verifyEmail.html",
                    "utf8"
                  );
                  ...
                  return res.json({
                    status: 1,
                    response: {
                      msg: "We have sent you a confirmation link to your email address. Please click on the link to verify/validate your account. Thank You!.",
                    },
                  });
                }
              }
...
router.post(
  ...
  function (req, res) {
    ...
    form.parse(req, function (err, fields, files) {
      ...
      var research_description = obj.research_description
        ? obj.research_description
        : "";
      var organization = obj.organization ? obj.organization : "";
      var level_of_education = obj.level_of_education
        ? obj.level_of_education
        : "";
      var occupation =
        obj.occupation && parseInt(obj.occupation) > 0 ? obj.occupation : null;
      var sector = obj.sector && parseInt(obj.sector) > 0 ? obj.sector : null;
      var academic_discipline =
        obj.academic_discipline && parseInt(obj.academic_discipline) > 0
          ? obj.academic_discipline
          : null;
      ...
      var other_academic_discipline = obj.other_academic_discipline
        ? obj.other_academic_discipline
        : "";
      var other_occupation = obj.other_occupation ? obj.other_occupation : "";
      var other_professional_interest_area =
        obj.other_professional_interest_area
          ? obj.other_professional_interest_area
          : "";
      var other_research_interest_area = obj.other_research_interest_area
        ? obj.other_research_interest_area
        : "";
          ...
          [
            function (done) {
              if (typeof files.image !== "undefined") {
                ...
                if (
                  ...
                  fs.rename(
                    tmp_path,
                    path.join(__dirname, env.USER_PATH + filename),
                    function (err) {
                      gm(__dirname + env.USER_PATH + filename)
                        .gravity("Center")
                        .thumb(
                          150,
                          150,
                          __dirname + env.USER_PATH_THUMB + filename,
                          100,
                          function (err, data) {
                            if (err) {
            ...
            function (overview, done1) {
              ...
              User.checkUserOrganization(organization, function (err, data) {
                let org_record = data;
                if (org_record) {
                  overview.organization = parseInt(
                    org_record[0].organization_id
                  );
                  done1(err, overview);
                } else {
  ...
  (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let record = {
        ...
        deactive_reason: req.body.deactive_reason
          ? req.body.deactive_reason
          : "",
      };
      ...
      User.updateuserByadmin(
        ...
        function (err, data) {
          if (err) {
            ...
            var htmlUser = fs.readFileSync(
              __dirname + "/templates/userRegistration/deactiveUser.html",
              "utf8"
            );
            var dynamicHtml = {
              logo: logo,
...
router.post("/sendInvite", (req, res) => {
  ...
  const event = {
    ...
    description:
      '<a href="http://www.bolderboulder.com/">Annual 10-kilometer run in Boulder, Colorado</a>.',
    ...
    attendees: [
      {
        name: "Adam Gibbons",
        email: "dipika.letsnurture@gmail.com",
        rsvp: true,
        partstat: "ACCEPTED",
